...
public class UploadUtil {
	...
	private static final String BOUNDARY = UUID.randomUUID().toString(); // 锟竭斤拷锟绞�
																			// 锟斤拷锟斤拷锟斤拷锟�
	 ...
	 * @param fileKey
	 *            锟斤拷锟斤拷页锟斤拷<input type=file name=xxx/> xxx锟斤拷锟斤拷锟斤拷锟斤拷锟�fileKey
	 * @param RequestURL
	 *            锟斤拷锟斤拷锟�URL
	 */
	public void uploadFile(String filePath, String fileKey, String RequestURL,
		...
		if (filePath == null) {
			sendMessage(UPLOAD_FILE_NOT_EXISTS_CODE, "锟侥硷拷锟斤拷锟斤拷锟斤拷");
			return;
		}
		try {
	...
	public void uploadFile(final File file, final String fileKey,
		...
		Log.i(TAG, "锟斤拷锟斤拷锟�URL=" + RequestURL);
		Log.i(TAG, "锟斤拷锟斤拷锟�fileName=" + file.getName());
		Log.i(TAG, "锟斤拷锟斤拷锟�fileKey=" + fileKey);
	...
	private void toUploadFile(File file, String fileKey, String RequestURL,
		...
		try {
			...
			sb = new StringBuffer();
			/**
			 * 锟斤拷锟斤拷锟截碉拷注锟解： name锟斤拷锟斤拷锟街滴�锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷要key 只锟斤拷锟斤拷锟�key 锟脚匡拷锟皆得碉拷锟斤拷应锟斤拷锟侥硷拷
			 * filename锟斤拷锟侥硷拷锟斤拷锟斤拷锟街ｏ拷锟斤拷锟斤拷锟斤拷缀锟斤拷锟斤拷 锟斤拷锟斤拷:abc.png
			 */
			sb.append(PREFIX).append(BOUNDARY).append(LINE_END);
			sb.append("Content-Disposition:form-data; name=\"" + fileKey
					+ "\"; filename=\"" + file.getName() + "\"" + LINE_END);
			sb.append("Content-Type:image/pjpeg" + LINE_END); // 锟斤拷锟斤拷锟斤拷锟矫碉拷Content-type锟斤拷锟斤拷要锟斤拷
																// 锟斤拷锟斤拷锟节凤拷锟斤拷锟斤拷锟剿憋拷锟斤拷募锟斤拷锟斤拷锟斤拷偷锟�
			sb.append(LINE_END);
			params = sb.toString();
			sb = null;
			...
			if (res == 200) {
				...
				sendMessage(UPLOAD_SUCCESS_CODE, "锟较达拷锟斤拷锟斤拷锟�" + result);
				return;
			} else {
				Log.e(TAG, "request error");
				sendMessage(UPLOAD_SERVER_ERROR_CODE, "锟较达拷失锟杰ｏ拷code=" + res);
				return;
			}
		} catch (MalformedURLException e) {
			sendMessage(UPLOAD_SERVER_ERROR_CODE,
					"锟较达拷失锟杰ｏ拷error=" + e.getMessage());
			e.printStackTrace();
			return;
		} catch (IOException e) {
			sendMessage(UPLOAD_SERVER_ERROR_CODE,
